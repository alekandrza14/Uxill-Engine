...
public class Uxill_Engine
{
   ...
   static public List<GameObject> pref4d28 = new List<GameObject>();
    ...
    static public WorldSave ws = new WorldSave();
    ...
    static public void Load(transform4d transform, GameObject[] gs,bool isstart)
    {
        ...
        if (Input.GetKeyDown(KeyCode.F2) && File.Exists(WorldData.path + "/worlddata.wd") && !isstart)
        {
            ...
            ws = new WorldSave();
            ws = JsonUtility.FromJson<WorldSave>(File.ReadAllText(WorldData.path + "/worlddata.wd"));
        ...
        if (File.Exists(WorldData.path + "/worlddata.wd") && isstart)
        {
    ...
    static public void Save(transform4d transform)
    {
        if (Input.GetKey(KeyCode.F1))
        {
            ...
            File.WriteAllText(WorldData.path+"/worlddata.wd",JsonUtility.ToJson(ws));
        }
    ...
    static public void Start(transform4d transform, GameObject[] gs)
    {
        ...
        Enginemanager e = GameObject.FindObjectsOfType<Enginemanager>()[0];
            ...
            pref4d38.Add(e.create(gs[3], new Vector3(0, 10, 0), Quaternion.identity));
            for (int i = 0; i < pref4d18.Count; i++)
            {
                ...
                if (getwposcamera(Mathf.FloorToInt(pos.w)).w == p4[i] && !pref4d38[0].GetComponent<player>().b)
                {
            ...
            for (int i3 = 0; i3 < p4.Count; i3++)
            {
                ...
                if (getwposcamera(Mathf.FloorToInt(transform.posistion.w)).w == p4[i3])
                {
                    ...
                    if (rt[i3] == null)
                    {
                        ...
                        window.texture = rt[i3] = new RenderTexture(s);
                        getwposcamera(Mathf.FloorToInt(transform.posistion.w)).c.targetTexture = rt[i3];
